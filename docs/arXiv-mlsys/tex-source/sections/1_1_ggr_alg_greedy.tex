...
%    \STATE $\text{tot\_len} \gets \text{len}(v)^2 + \sum_{\substack{c' \in \text{inferred\_cols}}} \left( \frac{\sum_{r \in R_v} \text{len}(T[r, c'])}{|R_v|} \right)^2$
    \STATE $\text{tot\_len} \gets \text{len}(v)^2 + \sum_{\substack{v' \in \text{inferred\_vals}}} \text{len}(v')^2$
...
\FUNCTION{\textsc{GGR}($T$, $FD$)}
    ...
    \STATE $A\_HC, A\_L \gets \textsc{GGR}(T[\text{rows} \setminus R_v, \text{cols}], FD)$
    \STATE $B\_HC, B\_L \gets \textsc{GGR}(T[R_v, \text{cols} \setminus b\_cols], FD)$
    \STATE $C\_HC \gets max\_HC$
    \STATE $S \gets A\_HC + B\_HC + C\_HC$
    \STATE $L \gets [b\_vals + B\_L[i] \mid i \in 1 \dots |R_v|] + A\_L$
